...
class UeditorListOSS
{
    ...
    {
        $objs = OSS::getAllObjectKey('chojer-uploads');
        foreach ($objs as $key => $value) {
            $tmp = end(explode('/', $value));
            if (preg_match("/\.(" . $allowFiles . ")$/i", $tmp)) {
                $files[] = array(
                    'url' => str_replace('https://', 'https://chojer-uploads.', Config::get('app.ossServer')) . '/' . $value,
